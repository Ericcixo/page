(function(){
  ...
  function renderDrawerLines(){
    ...
    if(cart.providers.length === 0 && cart.courses.length === 0){
      box.innerHTML = '<div style="text-align:center; padding:60px 20px; color:rgba(255,255,255,0.3);"><div style="font-size:40px; margin-bottom:10px">🛒</div>Tu carrito está vacío</div>';
      return;
    }
    ...
    [...new Set(cart.providers)].forEach(id => {
      html += `
      <div class="line" style="display:flex; justify-content:space-between; margin-bottom:16px; border-bottom:1px dashed rgba(255,255,255,0.1); padding-bottom:12px;">
        <div style="display:flex; gap:12px; align-items:center;">
           <div style="background:rgba(59,130,246,0.1); width:32px; height:32px; border-radius:8px; display:flex; align-items:center; justify-content:center; font-size:14px;">📦</div>
           <div>
              <div style="font-weight:600; font-size:15px;">${pNames[id] || 'Proveedor'}</div>
              <div style="font-size:12px; color:#94a3b8">Proveedor Verificado</div>
           </div>
        </div>
        <button class="btn-ghost" style="padding:6px; color:#f87171" onclick="window.RF.rmP('${id}')">✕</button>
      </div>`;
    });
    ...
    cart.courses.forEach(c => {
      ...
      <div class="line" style="display:flex; justify-content:space-between; margin-bottom:16px; border-bottom:1px dashed rgba(255,255,255,0.1); padding-bottom:12px;">
        <div style="display:flex; gap:12px; align-items:center;">
           <div style="background:rgba(139,92,246,0.1); width:32px; height:32px; border-radius:8px; display:flex; align-items:center; justify-content:center; font-size:14px;">📚</div>
           <div>
              <div style="font-weight:600; font-size:15px;">${cNames[c.id] || 'Guía PDF'}</div>
              <div style="font-size:12px; color:#94a3b8">Formato Digital</div>
           </div>
        </div>
        <button class="btn-ghost" style="padding:6px; color:#f87171" onclick="window.RF.rmC('${c.id}')">✕</button>
      </div>`;
    });
    
    // Cross-sell
    if(cart.providers.length > 0 && cart.courses.length === 0){
       html += `<div style="margin-top:24px; background:rgba(59,130,246,0.08); border:1px solid rgba(59,130,246,0.2); border-radius:12px; padding:14px;">
         <div style="font-size:13px; margin-bottom:10px; color:#bfdbfe">💡 <strong>Consejo Pro:</strong> Protege tu cuenta con el Curso de Seguridad.</div>
         <button class="btn-glow" style="padding:8px; font-size:13px; min-height:auto" onclick="window.RF.addC('c2')">Añadir (+1€)</button>
       </div>`;
    }
  ...
  function toast(msg, type='success'){
    const t = $('#toast');
    t.innerHTML = `<div style="background:${type==='error'?'rgba(127,29,29,0.9)':'rgba(6,78,59,0.9)'}; backdrop-filter:blur(8px); color:${type==='error'?'#fca5a5':'#a7f3d0'}; padding:12px 24px; border-radius:99px; box-shadow:0 10px 40px rgba(0,0,0,0.6); border:1px solid rgba(255,255,255,0.1); font-weight:600; font-size:14px;">${msg}</div>`;
  ...
  const checkoutBtn = $('#toCheckout');

  if(openBtn) openBtn.addEventListener('click', openDrawer);
  if(closeBtn) closeBtn.addEventListener('click', closeDrawer);
  if(overlay) overlay.addEventListener('click', closeDrawer); // Click fuera cierra
  
  if(clearBtn) clearBtn.addEventListener('click', () => { cart={providers:[],courses:[]}; saveCart(); });
  ...
  // --- COOKIES LOGIC (FUNCTIONAL) ---
  (function initCookies(){
    const banner = $('#cookieBanner');
    const acceptBtn = $('#cookieAccept');
    const rejectBtn = $('#cookieReject');
    
    const consent = localStorage.getItem('rf_cookie_consent');
