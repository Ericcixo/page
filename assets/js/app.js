(function(){
  ...
  window.RF = window.RF || {};
  // Valores por defecto si no carga config.js
  ...
  // Estado del Carrito (Safe Parse para evitar errores)
  ...
  function getTotals(){
    ...
    // Lógica de Bundle: 4 proveedores = 6€, el resto unitario
    let pPrice = 0;
  ...
  function renderDrawerLines(){
      ...
      <div class="line" style="display:flex; justify-content:space-between; margin-bottom:16px; border-bottom:1px dashed rgba(255,255,255,0.1); padding-bottom:12px;">
        <div style="display:flex; gap:12px; align-items:center;">
           ...
           <div>
              ...
              <div style="font-size:12px; color:#94a3b8">Verificado</div>
           </div>
        </div>
        <button class="btn-ghost" style="padding:8px; color:#f87171" onclick="window.RF.rmP('${id}')" aria-label="Eliminar">✕</button>
      </div>`;
    });
    ...
    cart.courses.forEach(c => {
      ...
      <div class="line" style="display:flex; justify-content:space-between; margin-bottom:16px; border-bottom:1px dashed rgba(255,255,255,0.1); padding-bottom:12px;">
        ...
        <button class="btn-ghost" style="padding:8px; color:#f87171" onclick="window.RF.rmC('${c.id}')" aria-label="Eliminar">✕</button>
      </div>`;
    });
  ...
  // --- ACTIONS (Globales para usar en HTML onclick) ---
  window.RF.addP = (id) => { 
  ...
  // Animaciones al hacer scroll (Lazy animations)
